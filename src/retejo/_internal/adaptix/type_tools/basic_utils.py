import types
import typing
from typing import Annotated, Any, ForwardRef, Generic, NewType, Protocol, TypeVar, TypedDict, Union
...
def is_named_tuple_class(tp) -> bool:
    return is_subclass_soft(tp, tuple) and all(hasattr(tp, attr_name) for attr_name in _NAMED_TUPLE_METHODS)
...
if HAS_PY_312:
    ...
    def is_user_defined_generic(tp: TypeHint) -> bool:
        return bool(get_type_vars(tp)) and (
            is_subclass_soft(strip_alias(tp), Generic) or isinstance(tp, typing.TypeAliasType)  # type: ignore[attr-defined, unused-ignore]
        )
else:
...
def is_generic(tp: TypeHint) -> bool:
    ...
    return (
        ...
        or (strip_alias(tp) in BUILTIN_ORIGIN_TO_TYPEVARS and tp is not type and not is_parametrized(tp))
        or (strip_alias(tp) == Annotated and tp != Annotated and is_generic(tp.__origin__))
    )
...
def is_generic_class(cls: type) -> bool:
    ...
    """
    return cls in BUILTIN_ORIGIN_TO_TYPEVARS or (
