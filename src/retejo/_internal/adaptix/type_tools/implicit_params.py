...
class ImplicitParamsGetter:
    ...
    def _process_type_var(self, type_var) -> TypeHint:
        ...
        if type_var.__constraints__:
            return create_union(
                tuple(self._process_limit_element(type_var, constraint) for constraint in type_var.__constraints__),
            )
    ...
    def get_implicit_params(self, origin) -> VarTuple[TypeHint]:
        ...
        return tuple(self._process_type_var(type_var) for type_var in type_vars)
