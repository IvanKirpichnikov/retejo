...
class _BasicNormType(BaseNormType, ABC):
    ...
    def __eq__(self, other):
        if isinstance(other, _BasicNormType):
            return self.origin == other.origin and self._args == other._args
...
class NormTV(BaseNormType):
    __slots__ = ("_limit", "_source", "_var", "_variance")
...
class TypeNormalizer:
    ...
    def _get_bound(self, type_var) -> Bound:
        return (
            Bound(ANY_NT)
            if (type_var.__bound__ is None or type_var.__bound__ is NoneType)
            else Bound(self.normalize(type_var.__bound__))
        )
    ...
    def _norm_type_var(self, tp, origin, args):
        if isinstance(origin, TypeVar):
            ...
            limit = (
                ...
                else namespaced._get_bound(origin)
            )
    ...
    def _norm_callable(self, tp, origin, args):
        if origin == c_abc.Callable:
            ...
            return _NormType(
                c_abc.Callable,
                (call_args, self.normalize(args[-1])),
                source=tp,
            )
    ...
    def _dedup_union_args(self, args: Iterable[BaseNormType]) -> Iterable[BaseNormType]:
        ...
        return [
            _replace_source_with_union(arg, sources) if len(sources) != 1 and isinstance(arg, _NormType) else arg
    ...
    def _norm_type(self, tp, origin, args):
        if is_subclass_soft(origin, type) and args:
            ...
            if norm.origin == Union:
                return _UnionNormType(
                    tuple(_NormType(type, (arg,), source=type[arg.source]) for arg in norm.args),
                    source=tp,
                )
    ...
    def _norm_other(self, tp, origin, args):
        ...
        if not (params or isinstance(origin, type) or origin in self.ALLOWED_ZERO_PARAMS_ORIGINS):
...
def soft_normalize_type(tp: TypeHint) -> BaseNormType:
    if not isinstance(tp, BaseNormType):
