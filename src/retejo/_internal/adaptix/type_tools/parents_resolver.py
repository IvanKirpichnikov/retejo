...
from typing import Any, Final, Generic, Protocol, TypeAlias

from retejo._internal.adaptix.type_tools import (  # type: ignore[attr-defined]
    normalize_type,
)
from retejo._internal.adaptix.type_tools.basic_utils import (
    get_type_vars_of_parametrized,
    is_generic,
    is_parametrized,
)
from retejo._internal.adaptix.type_tools.common import TypeHint
from retejo._internal.adaptix.type_tools.feature_requirement import (
    HAS_PY_312,
    HAS_TV_TUPLE,
    HAS_UNPACK,
)
from retejo._internal.adaptix.type_tools.fundamentals import (
    get_generic_args,
    get_type_vars,
    strip_alias,
)
from retejo._internal.adaptix.type_tools.implicit_params import fill_implicit_params
...
class ParentsResolver:
    ...
    def _unpack_args(self, args: TypeArgsTuple) -> TypeArgsTuple:
        if HAS_UNPACK and any(
            strip_alias(arg) == typing.Unpack  # type: ignore[attr-defined, unused-ignore]
            for arg in args
        ):
    ...
    def _get_parents(self, tp: TypeHint) -> list[TypeHint]:
        return [
            parent
            for parent in get_original_bases(tp)
